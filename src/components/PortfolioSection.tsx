...
const wordVariants   = { hidden: {}, visible: { transition: { staggerChildren: 0.06 } } };
...
function AnimatedTitle({ text }: { text: string }) {
  return (
    <motion.span className="block overflow-hidden" variants={wordVariants} initial="hidden" whileHover="visible">
    ...
    >
      <div className="container mx-auto px-6 lg:px-16">
        <div className="flex gap-12 items-start">
          ...
          {/* ── BAL STICKY OSZLOP: ragad amíg a kártyák scrolloznak ── */}
          <div
            style={{
              position:  'sticky',
              top:       '15vh',          // viewport tetejétől 15%
              width:     '340px',
              minWidth:  '280px',
              flexShrink: 0,
              zIndex:    30,
          ...
          >
            <ScrollMaskReveal delay={0}>
              <p className="section-muted text-sm tracking-[0.3em] uppercase mb-4">
                Portfólió
              </p>
            </ScrollMaskReveal>
            ...
            <h2 className="text-4xl md:text-5xl font-bold tracking-tight mb-6">
              <ScrollMaskReveal delay={0.12}>
                <span className="block">Válogatott</span>
              </ScrollMaskReveal>
              <ScrollMaskReveal delay={0.2}>
                <span className="block">Munkáim</span>
              </ScrollMaskReveal>
            </h2>
            ...
            <motion.p
              ...
              className="section-muted text-base font-light leading-relaxed"
            >
              A legújabb fogtechnikai munkáim közül válogatva –
              minden darab egyedi és precíz.
            </motion.p>

            {/* Gömb placeholder tér — ide lebeg be a Three.js gömb */}
            <div style={{ height: '280px', marginTop: '2rem' }} />
          </div>

          {/* ── JOBB SCROLLOZHATÓ KÁRTYÁK ── */}
          <div
            style={{
              flex:      1,
              zIndex:    20,
            ...
            <div className="columns-1 sm:columns-2 gap-5 space-y-5">
              {portfolioItems.map((item, i) => {
                ...
                return (
                  ...
                  >
                    <div
                      onClick={() => isVip ? navigate('/hidmunka') : undefined}
                    ...
                    >
                      ...
                      <div className="absolute inset-0 flex items-center justify-center z-0">
                        <div className={`section-muted text-sm tracking-widest uppercase ${
                          isVip ? 'opacity-20 text-amber-500' : 'opacity-40'
                        }`}>
                      ...
                      <div className={`absolute bottom-0 left-0 right-0 p-6 z-20 transition-all duration-500 ${
                        ...
                        <p className={`text-xs tracking-[0.2em] uppercase mb-2 ${
                          isVip ? 'text-amber-400 font-bold' : 'text-white/70'
                        }`}>
                          {item.category}
                        </p>
                        <h3 className={`font-semibold text-white ${
                          isVip ? 'text-3xl md:text-4xl drop-shadow-md' : 'text-lg'
                        }`}>
                          {isVip ? item.title : <AnimatedTitle text={item.title} />}
                        </h3>
                        {isVip && (
                          <p className="text-white/70 text-sm mt-3 font-light leading-relaxed">
                            Magas szintű anatómiai pontosság és esztétika,
                            amely csak sokéves tapasztalattal érhető el.
                          </p>
                        )}
                      </div>
                    </div>
