...
function MagneticLink({ href, className, children }: {
  ...
  return (
    <motion.a ref={ref} href={href} style={{ x: sx, y: sy, display: 'inline-flex' }}
      onMouseMove={(e) => {
...
const HeroSection = () => (
  <div
    className="flex flex-col justify-center px-8 md:px-16 lg:px-24 pt-32"
  ...
  >
    <div className="max-w-lg">
      ...
      <h1 className="text-6xl md:text-7xl font-bold tracking-tighter leading-[1.05] text-white mb-8">
        <div style={{ overflow: 'hidden' }}>
          <motion.span className="block"
            initial={{ y: '60px' }} animate={{ y: '0px' }}
            transition={{ type: 'spring', stiffness: 52, damping: 16, delay: 0.35 }}
          >Precizitás</motion.span>
        </div>
        <div style={{ overflow: 'hidden' }}>
          <motion.span className="block"
            initial={{ y: '60px' }} animate={{ y: '0px' }}
            transition={{ type: 'spring', stiffness: 52, damping: 16, delay: 0.5 }}
          >és Művészet.</motion.span>
        </div>
      </h1>

      <motion.p
        initial={{ opacity: 0, y: '16px' }} animate={{ opacity: 1, y: '0px' }}
      ...
      >
        ...
        >Munkáim megtekintése</MagneticLink>
